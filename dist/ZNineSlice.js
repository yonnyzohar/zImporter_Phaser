...
export class ZNineSlice extends Phaser.GameObjects.NineSlice {
    ...
    constructor(scene, x, y, texture, frame, nineSliceData, orientation) {
        super(scene, x, y, texture, frame, nineSliceData.width, nineSliceData.height, 
        ...
        Math.min(nineSliceData.left, Math.floor(nineSliceData.width / 2)), Math.min(nineSliceData.right, Math.floor(nineSliceData.width / 2)), Math.min(nineSliceData.top, Math.floor(nineSliceData.height / 2)), Math.min(nineSliceData.bottom, Math.floor(nineSliceData.height / 2)));
    ...
    applyTransform() {
        ...
        this.leftWidth = Math.min(this._nineSliceData.left, Math.floor(w / 2));
        this.rightWidth = Math.min(this._nineSliceData.right, Math.floor(w / 2));
        this.topHeight = Math.min(this._nineSliceData.top, Math.floor(h / 2));
        this.bottomHeight = Math.min(this._nineSliceData.bottom, Math.floor(h / 2));
        this.setSize(w, h);
        ...
        this.setPosition((this.currentTransform.x || 0) - scaleX * ownPivotX - parentPivotX, (this.currentTransform.y || 0) - scaleY * ownPivotY - parentPivotY);
