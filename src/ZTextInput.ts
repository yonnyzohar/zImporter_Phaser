...
export class ZTextInput extends ZContainer {
    ...
    private _preRenderHandler: (() => void) | null = null;

    // Local position from JSON — used by ZContainer.setOrigin() to apply parent pivot.
    public _baseX: number;
    public _baseY: number;
    ...
    private _createInput(scene: Phaser.Scene, data: TextInputData): void {
    ...
    private _applyBoxStyle(input: HTMLInputElement, box: any): void {
        if (!box) return;
     ...
     */
    private _updateDOMPosition(): void {
        if (!this.inputElement) return;

        const canvas = this._scene.sys.game.canvas as HTMLCanvasElement;
    ...
    private _findAncestorClip(): { tl: { x: number; y: number }; br: { x: number; y: number } } | null {
        let node: any = this.parentContainer;
        while (node) {
            if (node._clipWorld) return node._clipWorld;
